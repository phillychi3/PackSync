...
export const trip = sqliteTable('trip', {
  ...
  status: text('status', { enum: ['planning', 'ongoing', 'completed'] }).notNull().default('planning'),
...
export const tripMember = sqliteTable('trip_member', {
  id: text('id').primaryKey().$defaultFn(() => crypto.randomUUID()),
  tripId: text('trip_id').notNull().references(() => trip.id, { onDelete: 'cascade' }),
  userId: text('user_id').notNull().references(() => user.id, { onDelete: 'cascade' }),
  role: text('role', { enum: ['owner', 'admin', 'member'] }).notNull().default('member'),
  joinedAt: integer('joined_at', { mode: 'timestamp_ms' })
    .default(sql`(cast(unixepoch('subsecond') * 1000 as integer))`).notNull(),
}, (t) => [
  uniqueIndex('trip_member_unique').on(t.tripId, t.userId),
]);
...
export const invitation = sqliteTable('invitation', {
  ...
  index('invitation_trip_idx').on(t.tripId),
]);
...
export const place = sqliteTable('place', {
  ...
  index('place_trip_idx').on(t.tripId),
]);
...
export const scheduleItem = sqliteTable('schedule_item', {
  ...
  index('schedule_item_trip_date_idx').on(t.tripId, t.date),
]);
...
export const bill = sqliteTable('bill', {
  ...
  index('bill_trip_idx').on(t.tripId),
]);
...
export const settlement = sqliteTable('settlement', {
  ...
  index('settlement_trip_idx').on(t.tripId),
]);
...
export const packingItem = sqliteTable('packing_item', {
  id: text('id').primaryKey().$defaultFn(() => crypto.randomUUID()),
  listId: text('list_id').notNull().references(() => packingList.id, { onDelete: 'cascade' }),
  name: text('name').notNull(),
...
export const criticalItemConfirmation = sqliteTable('critical_item_confirmation', {
  id: text('id').primaryKey().$defaultFn(() => crypto.randomUUID()),
  criticalItemId: text('critical_item_id').notNull().references(() => criticalItem.id, { onDelete: 'cascade' }),
  userId: text('user_id').notNull().references(() => user.id, { onDelete: 'cascade' }),
  scheduleItemId: text('schedule_item_id').references(() => scheduleItem.id, { onDelete: 'set null' }),
  confirmedAt: integer('confirmed_at', { mode: 'timestamp_ms' })
    .default(sql`(cast(unixepoch('subsecond') * 1000 as integer))`).notNull(),
}, (t) => [
  index('critical_confirmation_item_idx').on(t.criticalItemId),
]);
...
export const conversation = sqliteTable('conversation', {
  ...
  index('conversation_trip_user_idx').on(t.tripId, t.userId),
]);
